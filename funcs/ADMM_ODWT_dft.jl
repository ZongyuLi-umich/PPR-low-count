# ADMM_ODWT_dft.jl
"""
This code implements regularized ADMM (with ODWT regularizer) for the case
when the system matrix A is DFT, so A'A is a simple digonal matrix.
Input:
`A`: M x N complex matrix, where the ith row is a_i'
`ATA_inv`: N x N diagonal matrix, inverse of A'A
...
`reg`: parameter in ODWT regularizer
...
"""
function ADMM_ODWT_dft(A::AbstractMatrix{<:Number},
        ...
        for iter = 1:niter
            ...
            # For x update
            if xhow === :real
                tilde_x = ATA_inv * real((A' * (v + η)))
            elseif xhow === :complex
                tilde_x = ATA_inv * (A' * (v + η))
            else
                throw("unknown xhow")
            end

            x = g_prox(tilde_x, diag(ATA_inv)/ρ)
            Ax_new = A * x
