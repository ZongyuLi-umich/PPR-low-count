...
save("./result/A_ETM_ODWT_speed/optimal_init_new_exp_count="*string(avg_count)*"_M="*string(M)*".jld2",
...
# plot data
# M = 80000
data = load("./result/A_ETM_ODWT_speed/optimal_init_new_exp_count=0.25_M="*string(M)*".jld2")
...
scatter(1e3 * remove_gap(grab_time(data["cout_admm_ODWT"], r_time)),
                        ...
                        color = "blue",label = "ADMM ODWT", legendfontsize = 12,
                        xguidefontsize=15, yguidefontsize=15, xlabel = "Time (ms)", ylabel = L"\log(f(x))")
...
savefig("./result/A_ETM_ODWT_speed/optimal_init_new_exp_count=0.25_M="*string(M)*".pdf")
# NRMSE
