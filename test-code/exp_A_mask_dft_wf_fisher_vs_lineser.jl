...
save(cpt*"result/A_mask_dft_wf_ls_vs_fisher/optimal_init_wf_2D_stepsize_num_mask="*string(num_mask)*"count="*string(avg_count)*".jld2",
...
data = load(cpt*"result/A_mask_dft_wf_ls_vs_fisher/optimal_init_wf_2D_stepsize_num_mask=20count=0.25.jld2")
...
savefig(cpt*"result/A_dft_wf_ls_vs_fisher/optimal_init_wf_empirical_stepsize_cost_fun_2D_num_mask="*string(num_mask)*"count="*string(avg_count)*".pdf")
scatter(1e3 * remove_gap(grab_time(data["cout_wf_ls_notrun"], r_time)),
        log10.(grab_cost(data["cout_wf_ls_notrun"], r_time) .+ 2e6),
        color = "red", legendfontsize = 12, label = "Backtracking",
        xlabel = "Time (ms)", xlims = (0, 5e4), ylabel = L"\log_{10}(f(x)+ 2e6)",
        right_margin = 15Plots.mm, legend = :right)
scatter!(1e3 * remove_gap(grab_time(data["cout_wf_optim_gau_notrun"], r_time)),
         log10.(grab_cost(data["cout_wf_optim_gau_notrun"], r_time) .+ 2e6),
         color = "magenta", legendfontsize = 12, xlims = (0, 5e4),
         label = "Optim Gau", legend = :right)
scatter!(1e3 * remove_gap(cal_time_lbfgs_mean(data["results_LBFGS"])),
         log10.(cal_cost_lbfgs_mean(data["results_LBFGS"]) .+ 2e6),
         color = "purple", xlims = (0, 5e4), legendfontsize = 12,
         label = "LBFGS", legend = :right)
scatter!(1e3 * remove_gap(grab_time(data["cout_wf_fisher_notrun"], r_time)),
         log10.(grab_cost(data["cout_wf_fisher_notrun"], r_time) .+ 2e6),
         color = "green", xlims = (0, 5e4), legendfontsize = 12,
...
scatter!(subplot, 1e3 * remove_gap(grab_time(data["cout_wf_ls_notrun"], r_time)),
         ...
         ylabel = "PSNR (dB)", xlims = (0, 5e4), right_margin = 15Plots.mm)
scatter!(subplot, 1e3 * remove_gap(grab_time(data["cout_wf_optim_gau_notrun"], r_time)),
         ...
         legendfontsize = 12, xlims = (0, 5e4), markershape = :rect,
         color = "magenta", label = "")
...
savefig(cpt*"result/A_mask_dft_wf_ls_vs_fisher/optimal_init_wf_stepsize_cost_fun_psnr_2D_num_mask="*string(num_mask)*"count="*string(avg_count)*".pdf")
