...
for r = 1:r_time
    ...
    global x0_spectral = scale_factor * x0_spectral
    ...
    g_unreg!(y, x) = copyto!(y, A' * grad_phi.(A * x, y_pos, b))
    ...
    g_reg!(y, x) = copyto!(y, A' * grad_phi.(A * x, y_pos, b) + β * T' * grad_huber.(T * x, α))
    grad_reg!(y, Ax, Tx) = copyto!(y, A' * grad_phi.(Ax, y_pos, b) + β * T' * grad_huber.(Tx, α))
...
save("../result/A_ETM_TV_speed/optimal_init_exp_10times_all_2D_count="*string(avg_count)*"_M="*string(M)*".jld2",
...
# plot data
data = load("../result/A_ETM_TV_speed/optimal_init_exp_10times_all_2D_count="*string(avg_count)*"_M="*string(M)*".jld2")
# unregularized algorithms
...
plot(1e3 * remove_gap(grab_time(data["cout_wf_ls_notrun"], r_time)),
            ...
            xlabel = "Time (ms)", xlims = (0, 700), ylabel = L"\log_{10}(f(x))",
...
plot!(1e3 * remove_gap(grab_time(data["cout_wf_optim_gau_notrun"], r_time)),
            ...
            markershape = :circle, markersize = 4, linealpha = 0.5, linewidth=0.5,
            color = "magenta", legendfontsize = 12, xlims = (0, 700),
...
plot!(1e3 * remove_gap(cal_time_lbfgs_mean(data["results_LBFGS"])),
            ...
            markershape = :circle, markersize = 4, linealpha = 0.5, linewidth=0.5,
            color = "purple", xlims = (0, 700), legendfontsize = 12,
...
plot!(1e3 * remove_gap(grab_time(data["cout_wf_fisher_notrun"], r_time)),
            ...
            markershape = :circle, markersize = 4, linealpha = 0.5, linewidth=0.5,
            color = "green", xlims = (0, 700), legendfontsize = 12,
...
plot!(subplot, 1e3 * remove_gap(grab_time(data["cout_wf_ls_notrun"], r_time)),
                    ...
                    ylabel = "PSNR (dB)", xlims = (0, 700), right_margin = 15Plots.mm,
                    xlabelfontsize = 16, ylabelfontsize = 16)
plot!(subplot, 1e3 * remove_gap(grab_time(data["cout_wf_optim_gau_notrun"], r_time)),
                    ...
                    markersize = 4, linealpha = 0.5, linewidth=0.5,
                    legendfontsize = 12, xlims = (0, 700), markershape = :rect,
                    color = "magenta", label = "")
...
plot!(subplot, 1e3 * remove_gap(grab_time(data["cout_wf_fisher_notrun"], r_time)),
                    ...
                    xlims = (0, 700))
#
savefig("../result/A_ETM_wf_ls_vs_fisher/wf_stepsize_optimal_init_10times_cost_fun_psnr_2D_count="*string(avg_count)*".pdf")
...
plot(remove_gap(grab_time(data["cout_admm_TV"], r_time)),
            ...
            xlabel = "Time (s)", xlims = (0, 2), ylabel = L"\log_{10}(f(x))",
...
plot!(remove_gap(cal_time_lbfgs_mean(data["results_LBFGS_TV"])),
            ...
            markershape = :circle, markersize = 4, linealpha = 0.5, linewidth=0.5,
            color = "purple", legendfontsize = 12, xlims = (0, 2), label = "LBFGS-TV")
...
plot!(remove_gap(cal_time_wf_inplace(time_wf_inplace)),
            log10.(cal_cost_wf_inplace(cost_wf_inplace)), xlims = (0, 2),
            markershape = :circle, markersize = 4, linealpha = 0.5, linewidth=0.5,
...
plot!(subplot, remove_gap(grab_time(data["cout_admm_TV"], r_time)),
            ...
            legendfontsize = 12, xlims = (0, 2), markershape = :rect,
            markersize = 4, linealpha = 0.5, linewidth=0.5,
...
plot!(subplot, remove_gap(cal_time_wf_inplace(time_wf_inplace)),
            ...
            markersize = 4, linealpha = 0.5, linewidth=0.5,
            color = "green", label = "")
savefig("../result/A_ETM_TV_speed/optimal_init_cost_fun_2D_10times_count="*string(avg_count)*".pdf")
