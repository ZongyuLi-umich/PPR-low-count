...
r_time = 50
xhow = :complex
avg_count = 2 # 0.25, 1, 2
niter = 200

num_mask = 20
K = 2 * N - 1
pad = x -> vcat(x, zeros(eltype(x), K - N))
unpad = x -> x[1:N]
...
# Define xtrue
xtrue = ComplexF32.((cumsum(mod.(1:N, 30) .== 0) .- 1.5) + im * (cumsum(mod.(1:N, 10) .== 0) .- 4.5))
...
for r = 1:r_time
        ...
        # LBFGS
        results = optimize(f, g!, x0_spectral, LBFGS(), Optim.Options(store_trace = true,
                                                                      show_trace = false,
                                                                      extended_trace = true))
        # get time Optim.trace(results)[1].metadata["time"]
...
save("./result/A_fft_huber_TV/A_fft_huber_TV_"*string(xhow)*".jld",
