...
for r = 1:r_time
        global A1 = LinearMapAA(x -> vec(fft(pad(reshape(x, N, N)))),
...
save("./result/A_mask_dft_wf_vs_poi/optimal_init_2D_gau_vs_poi_mask="*string(num_mask)*"avgcount="*string(avg_count)*".jld2",
...
# plot data
data = load("./result/A_mask_dft_wf_vs_poi/optimal_init_2D_gau_vs_poi_avgcount="*string(avg_count)*".jld2")
...
savefig("./result/A_mask_dft_wf_vs_poi/optimal_init_2D_wf_gau.pdf")
nrmse_pois = nrmse(vec(xout_wf_pois))
jim(xout_wf_pois, "WF Poisson, NRMSE = $nrmse_pois%")
savefig("./result/A_mask_dft_wf_vs_poi/optimal_init_2D_wf_pois.pdf")
nrmse_pois_huber = nrmse(vec(xout_wf_pois_huber))
jim(xout_wf_pois_huber, "WF Poisson TV, NRMSE = $nrmse_pois_huber%")
savefig("./result/A_mask_dft_wf_vs_poi/optimal_init_2D_wf_pois_TV.pdf")
